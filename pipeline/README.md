...
- `envision/` writes the raw original image, processed image, visual impression, uncertainty map, and metadata.
- `attend/` consumes the processed Envision image, visual impression, uncertainty map, and writes the selected counterfactual region plus `attend_result.json`.
- `respond/` consumes the processed Envision image and Attend result, then writes regular and EnAR answers.
