...
DEFAULT_SCRIPT_CONFIG = Path("EnAR/work_scripts/interactive_padded_visual_top_p_debug.yaml")
...
def main() -> int:
    ...
    print(f"attend_result_json: {config.attend_result_json}")
    print(f"top_p: {config.top_p}, temperature: {config.temperature}, do_sample: {config.do_sample}")
    print(f"padding_strategy: {padded_result.padding_meta['actual_strategy']}")
    print(f"padded visual tokens: {padded_result.padding_meta['replaced_count']}")
    print(f"selected_vision_token_indices: {padded_result.padding_meta['selected_vision_token_indices']}")
    print_padding_diagnostics(
    ...
    if args.print_prompt_token_map:
        print_prompt_token_map(
            ...
            padded_visual_indices=padded_result.padding_meta["selected_vision_token_indices"],
        )
    print("Press Enter to accept the selected token; type a token id to force it; type q to quit.")
    print()

    run_interactive_padded_loop(
        config=config,
        args=args,
        model=components.model,
        tokenizer=components.tokenizer,
        ...
        visual_embeddings_padded=padded_result.visual_embeddings_padded,
    )
    return 0
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        parents=[pre_parser],
        description="Interactively inspect next-token probabilities using only Attend-padded visual embeddings."
    )
    ...
    parser.add_argument(
        ...
        default=defaults.get("padding_strategy"),
    )
...
def apply_preset(config: RespondConfig, preset: str) -> None:
    ...
    # Paper-style Respond reproduction defaults for LLaVA-1.5-7B debugging:
    # inspect the padded visual branch created by replacing attended visual
    # embeddings with the model pad-token embedding.
    config.do_sample = False
    config.temperature = 1.0
    config.top_p = 1.0
    config.use_apc = False
    config.apc_beta = 0.1
    config.padding_strategy = "pad_token_embedding"
...
def run_interactive_padded_loop(
    ...
    with torch.inference_mode():
        for step in range(config.max_new_tokens):
            ...
            selection = selector.select(logits)
            rows = top_p_rows(
                logits,
                tokenizer,
                top_p=config.top_p,
...
def print_prompt_token_map(
    ...
    padded_visual_indices: list[int],
) -> None:
    ...
    padded_set = {int(idx) for idx in padded_visual_indices}
    print()
    ...
    for pos, token_id in enumerate(ids):
        if pos in image_position_to_visual:
            ...
            token = "<pad>" if visual_idx in padded_set else "<image>"
...
def print_padding_diagnostics(
    ...
    image_positions = encoded.image_token_positions
    selected = [int(idx) for idx in padding_meta.get("selected_vision_token_indices", [])]
    sequence_positions = [
        image_positions[idx] for idx in selected if 0 <= idx < len(image_positions)
    ]
    print()
    print("Padding diagnostics:")
    print(f"num_image_tokens: {len(image_positions)}")
    print(f"len(image_token_positions): {len(image_positions)}")
    print(f"image_token_positions[:10]: {image_positions[:10]}")
    print(f"image_token_positions[-10:]: {image_positions[-10:]}")
    print(f"selected_visual_indices_count: {len(selected)}")
    print(f"selected_visual_indices[:10]: {selected[:10]}")
    print(f"selected_visual_indices[-10:]: {selected[-10:]}")
    print(f"selected_sequence_positions[:10]: {sequence_positions[:10]}")
    print(f"selected_sequence_positions[-10:]: {sequence_positions[-10:]}")
    print(f"ignored_vision_token_indices: {padding_meta.get('ignored_vision_token_indices', [])}")
    print(
        "pad:",
    ...
    print("eos:", getattr(tokenizer, "eos_token", None), getattr(tokenizer, "eos_token_id", None))
    print("unk:", getattr(tokenizer, "unk_token", None), getattr(tokenizer, "unk_token_id", None))
    print("image_features_stats:", tensor_stats(visual_embeddings))
    print("replacement_stats:", padding_meta.get("replacement_stats"))
    print("pad_token_meta:", padding_meta.get("pad_token"))
    print()
